...
def record_agreement(file_name, name):
    ...
    """
    dt_now = datetime.now()
    with open('data/{}_TIPI_agreement.csv'.format(file_name), 'a') as f:
        f.write('{},{}\n'.format(name, dt_now))
...
def record_answer_time(personal_code, category, question, value, elapsed_ms):
    ...
    回答時間記録
    """
    with open('data/{}_TIPI_answer_time.csv'.format(personal_code), 'a') as f:
...
def post_recorded_files(file_name):
    ...
    記録データポスト
    """
    url = 'https://cloud.bmk.hiroshima-u.ac.jp/remote.php/dav/files/fc84411e-fa2b-103a-98c8-2970a9af346f/LabExp/TIPI/'
    id = 'dcnlabexp'
    pswd = 'I80rD0Ke5ep_'
    file0 = {
        'agreement': open('data/{}_TIPI_agreement.csv'.format(file_name), 'rb')
            }
    end_point = url + file_name + '_TIPI_agreement.csv'
    res = requests.put(end_point, auth=(id, pswd), files=file0)
    file = {
       'answer_time': open('data/{}_TIPI_answer_time.csv'.format(file_name), 'rb')
        }
    end_point = url + file_name + '_TIPI_answer_time.csv'
